...
- [<a name="angular-2-performance-checklist"></a>Angular パフォーマンスチェックリスト](#angular-パフォーマンスチェックリスト)
  - [<a name="introduction"></a>序論](#序論)
  - [<a name="table-of-content"></a>目次](#目次)
  - [<a name="network-performance"></a>ネットワークパフォーマンス](#ネットワークパフォーマンス)
    - [<a name="bundling"></a>バンドリング](#バンドリング)
    - [<a name="minification-and-dead-code-elimination"></a>ミニファイと不要コードの削除](#ミニファイと不要コードの削除)
    - [<a name="remove-template-whitespace"></a>テンプレート中の空白を削除](#テンプレート中の空白を削除)
    - [<a name="tree-shaking"></a>ツリーシェイキング](#ツリーシェイキング)
    - [<a name="tree-shakeable-providers"></a>ツリーシェイキング可能なプロバイダ](#ツリーシェイキング可能なプロバイダ)
    - [<a name="ahead-of-time-aot-compilation"></a>Ahead-of-Time (AoT) コンパイル](#ahead-of-time-aot-コンパイル)
    - [<a name="compression"></a>圧縮](#圧縮)
    - [<a name="pre-fetching-resources"></a>リソースの事前読込み](#リソースの事前読込み)
    - [<a name="lazy-loading-of-resources"></a>リソースの遅延読み込み](#リソースの遅延読み込み)
    - [<a name="dont-lazy-load-the-default-route"></a>デフォルトのルートを遅延読み込みにしない](#デフォルトのルートを遅延読み込みにしない)
    - [<a name="caching"></a>キャッシング](#キャッシング)
    - [<a name="use-application-shell"></a>アプリケーションシェルを使う](#アプリケーションシェルを使う)
    - [<a name="use-service-workers"></a>サービスワーカーを使う](#サービスワーカーを使う)
  - [<a name="runtime-optimizations"></a>実行時の最適化](#実行時の最適化)
    - [<a name="use-enableprodmode"></a> `enableProdMode`を使う](#-enableprodmodeを使う)
    - [<a name="ahead-of-time-compilation"></a>Ahead-of-Time コンパイル](#ahead-of-time-コンパイル)
    - [<a name="web-workers"></a>Webワーカー](#webワーカー)
    - [<a name="server-side-rendering"></a>サーバーサイドレンダリング](#サーバーサイドレンダリング)
    - [<a name="change-detection"></a>変更検出](#変更検出)
      - [<a name="changedetectionstrategyonpush"></a>`ChangeDetectionStrategy.OnPush`](#changedetectionstrategyonpush)
      - [<a name="detaching-the-change-detector"></a>Change Detectorの切り離し](#change-detectorの切り離し)
      - [<a name="run-outside-angular"></a>Run outside Angular](#run-outside-angular)
    - [<a name="use-pure-pipes"></a>純粋なpipeを使う](#純粋なpipeを使う)
    - [<a name="ngfor-directive"></a>`*ngFor` ディレクティブ](#ngfor-ディレクティブ)
      - [<a name="use-trackby-option"></a>`trackBy` オプションを使う](#trackby-オプションを使う)
      - [<a name="minimize-dom-elements"></a>DOM要素を小さくする](#dom要素を小さくする)
    - [<a name="optimize-template-expressions"></a>テンプレート中の式を最適化する](#テンプレート中の式を最適化する)
- [<a name="conclusion"></a>終わりに](#終わりに)
- [<a name="contributing"></a>貢献する](#貢献する)
- [License](#license)
